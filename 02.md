...
## `did:ckb` Method Local ID Extension

`draft` `optional` `proposer: Ian Yang (Cryptape)`

This document defines the Local ID Extension to the `did:ckb` method. A Local ID in the `did:ckb` method provides a lightweight, off-chain identity mechanism that enables users to establish and migrate CKB DIDs without immediately incurring the costs and consensus requirements of on-chain operations. Local IDs are particularly useful for scenarios where only limited consensus (such as within a small group) is needed, and they can later be migrated to a full on-chain `did:ckb` DID. When a Local ID is migrated, its reference is recorded in the `local_id` field of the newly created `did:ckb` DID Metadata Cell.
...
A user MUST perform following steps to migrate a Local ID while creating the `did:ckb` DID.

1. A user MUST provide a Local ID to a `did:ckb` controller while creating the `did:ckb` DID.
...
When a Local ID is migrated to a `did:ckb` DID, the new DID Metadata Cell includes the Local ID in its `local_id` field. The cell's type script (`did-ckb-ts`) MUST verify that:
...
The associated Local ID is immutable. The `did-ckb-ts` Type Script implementation MUST validate each update transaction to ensure that the Local ID cannot be altered:

1. The Local ID stored as the field `local_id` must be the same in the input and output DID Metadata Cell in an update transaction.

The authorization for Local ID migration is embedded in the CKB transaction's `witnesses` array. The position of the witness data corresponds to the output index of the DID Metadata Cell in the transaction's outputs array. Specifically, if the DID Metadata Cell is at `tx.outputs[i]`, the corresponding witness data is stored in `tx.witnesses[i]`. To avoid conflicts with input lock scripts and type scripts, the witness data is structured using the [WitnessArgs](https://docs.nervos.org/docs/tech-explanation/witness#witnessargs) molecule table, with the relevant data placed in the `output_type` field.

### 2.2 Resolution Process

Before migration, the resolver MUST use the algorithm specified by the Local ID’s method name to resolve the DID Metadata. After migration, the resolver MUST redirect resolution requests for the Local ID to its corresponding `did:ckb` DID.

A resolver MUST index all on-chain `did:ckb` DID Metadata Cells and look for the Local ID in their `local_id` field. The process is as follows:

- Process all newly created DID Metadata Cells in order of transaction and output index.
- For a given Local ID, the first `did:ckb` DID Metadata Cell that references it in `local_id` becomes the initial candidate for migration.
...
- **`sig`**: A signature produced by one of the `rotationKeys` from the last operation in `history`, authorizing the creation of the `did:ckb` DID on-chain.
...
The `did-ckb-ts` Type Script MUST perform the following validations:
...
- The PLC method-specific-id MUST be computed from first operation in `history` and MUST match the corresponding Local ID in the `DidCkbData`.
...
`DidCkbWitness` wraps the authorization information for the Local ID referenced in `local_id_authorization`.
...
```javascript
{
  ...
  "outputs": [
    // The new created DID Metadata Cell
    {
      "capacity": "0x258",
      "lock": {
        "args": "0x",
...
The cell data is the Molecule encoding of `DidCkbData` using `DidCkbDataV1` that `local_id` is `did:plc:bxvfvvygwbcnbmknn73t6pbu` in UTF-8 and `document` is the DAG-CBOR encoding of the following JSON object:
