...
This document defines the Local ID Extension to the `did:ckb` method. A Local ID in the `did:ckb` method provides a lightweight, off-chain identity mechanism that enables users to establish and migrate CKB DIDs without immediately incurring the costs and consensus requirements of on-chain operations. Local IDs are particularly useful for scenarios where only limited consensus (such as within a small group) is needed, and they can later be migrated to a full on-chain `did:ckb` DID. When a Local ID is migrated, its reference is recorded in the `local_id` field of the newly created `did:ckb` DID Document Cell.
...
When a Local ID is migrated to a `did:ckb` DID, the new DID Document Cell includes the Local ID in its `local_id` field. The cell's type script (`did-ckb-ts`) MUST verify that:
...
Before migration, the resolver MUST use the algorithm specified by the Local ID’s method name to resolve the DID document. After migration, the resolver MUST redirect resolution requests for the Local ID to its corresponding `did:ckb` DID.

A resolver MUST index all on-chain `did:ckb` DID Document Cells and look for the Local ID in their `local_id` field. The process is as follows:

- Process all newly created DID Document Cells in order of transaction and output index.
- For a given Local ID, the first `did:ckb` DID Document Cell that references it in `local_id` becomes the initial candidate for migration.
